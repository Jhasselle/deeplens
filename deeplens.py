...
def write_image_to_s3(img):
    ...
    file_name = '000112.jpg'
        # You can contorl the size and quality of the image
    encode_param=[int(cv2.IMWRITE_JPEG_QUALITY),90]
    _, jpg_data = cv2.imencode('.jpg', img, encode_param)

    mac = open('/sys/class/net/mlan0/address').readline()fif
    response = s3.put_object(ACL='public-read', Body=mac,Bucket='publicbucketforliverefresh',Key='mac.txt')
    response = s3.put_object(ACL='public-read', Body=jpg_data.tostring(),Bucket='publicbucketforliverefresh',Key=file_name)
    
    image_url = https://s3.amazonaws.com/publicbucketforliverefresh//'+file_name
    return image_url
        ...
        while True:
            ...
            for obj in parsed_inference_results[model_type]:
                ...
                if obj['prob'] > detection_threshold:
                    ...
                    cv2.rectangle(frame, (xmin, ymin), (xmax, ymax), (15, 15, 255), 5)
                    ...
                    # and tickness
                    cv2.putText(frame, 'MEAT CREATURE DETECTED {:.2f}%'.format(obj['prob'] * 100),
                                (xmin, ymin-text_offset),
                                cv2.FONT_HERSHEY_SIMPLEX, 2.5, (255, 15, 30), 6)
                    labelString = obj['label']
                    cv2.putText(frame, 'LABEL: {0}'.format(labelString),
                                (xmin, ymin-text_offset - 100),
                                cv2.FONT_HERSHEY_SIMPLEX, 2.5, (255, 15, 30), 6)
                                
                                
                    # Store label and probability to send to cloud
                    if obj['prob'] > 0.5:
                        cv2.putText(frame, 'Good enough.',
                                write_image_to_s3(img)
                                (xmin, ymin-text_offset + 100),
                                cv2.FONT_HERSHEY_SIMPLEX, 2.5, (255, 15, 30), 6)
                        cloud_output[output_map[obj['label']]] = obj['prob']
                    else:
                        cv2.putText(frame, 'Meh.',
                                (xmin, ymin-text_offset + 100),
                                cv2.FONT_HERSHEY_SIMPLEX, 2.5, (255, 15, 30), 6)
